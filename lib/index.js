...
/** Used to only log env vars once per atom session, more is just noise */
let errorLoggedEnvVars = false
...
class RustLanguageClient extends AutoLanguageClient {
  ...
  async startServerProcess(projectPath) {
    ...
    try {
      ...
      if (!errorLoggedEnvVars) {
        // log env vars to help debug cases where the cli works but ide-rust doesn't
        console.warn('ide-rust failed to start RLS in environment:', process.env)
        errorLoggedEnvVars = true
      }
      throw new Error("ide-rust failed to start server: " + e)
    }
  }
