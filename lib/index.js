...
function checkToolchain () {
  return new Promise((resolve, reject) => {
    exec("rustup toolchain list").then((results) => {
...
function checkRls() {
  return exec('rustup component list --toolchain nightly').then((results) => {
    ...
    // Don't have RLS
    return exec('rustup component add rls --toolchain nightly').then(() =>
      exec('rustup component add rust-src --toolchain nightly')
    ).then(() =>
...
class RustLanguageClient extends AutoLanguageClient {
  ...
  startServerProcess () {
    ...
    return checkToolchain().then((toolchain_) => {
      ...
      console.log("[ide-rust] All dependencies installed and ready to go")
      return cp.spawn('rustup', ['run', 'nightly', 'rls'], {env: {
        PATH: getPath(),
        // TODO: Support using an existing var if present
        RUST_SRC_PATH: path.join(os.homedir(), '.multirust/toolchains/' + toolchain + '/lib/rustlib/src/rust/src/'),
      }})
    })
  }
}
